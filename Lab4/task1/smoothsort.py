...
def _split_tree(root_idx, order, leo):
...
def _heapify_up_down(seq, i, heap_orders, leo):
    """
    ...
    while order >= 2:
        right_root, right_order, left_root, left_order = _split_tree(i, order, leo)
...
def smoothsort(seq):
    """
    Теперь уже сама smoothsort.
    """
    n = len(seq)
    if n < 2:
        return seq

    leo = _build_leonardo(n)
    heap_orders = []
    ...
    for i in range(n):
        ...
        _heapify_up_down(seq, i, heap_orders, leo)

    # Разбор леса — вытаскиваем элементы в порядке, шаг за шагом большие элементы оказываются справа и мы их вытаскиваем
    for i in range(n - 1, -1, -1):
        if heap_orders[-1] < 2:
            ...
            _heapify_up_down(seq, left_root, heap_orders, leo)

            heap_orders.append(right_order)
            _heapify_up_down(seq, right_root, heap_orders, leo)
